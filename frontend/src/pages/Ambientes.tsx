...
const estiloEstado: Record<FilaAmbiente['estado'], string> = {
  ...
  'Ocupado ahora': 'bg-orange-50 text-orange-700',
}
      ...
      <div className="overflow-hidden rounded-xl border border-slate-200 bg-white">
        <table className="w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-slate-100">
            {AMBIENTES.map((a) => (
              <tr key={a.codigo} className="hover:bg-slate-50">
                <td className="px-4 py-3 font-medium text-slate-900">{a.codigo}</td>
                <td className="px-4 py-3 text-slate-600">{a.sede}</td>
                <td className="px-4 py-3 text-slate-600">{a.coordinacion}</td>
